...
app = Flask(__name__)
app.config['SECRET_KEY'] = 'wuO6KSYOcwO2jesuI3hPMY5Fzi81QpX6'
...
def quiz_load(quiz_id):
    if request.method == 'GET':
        ...
        if existing_score:
            ...
            quiz_object = json.loads(existing_score.answer_json)
            ...
            return render_template("quiz_results.html", quiz=quiz_object)
...
def rate_and_submit(quiz_id):
    ...
    existing_suggestion = Score.query.filter_by(user_id=current_user.get_db_id()).filter_by(quiz_id=quiz_id).first()
...
@app.route('/quiz_suggestions/<int:quiz_id>', methods=["GET"])
@login_required
def quiz_suggestions(quiz_id):
    quiz_to_load = Quiz.query.get_or_404(quiz_id)
    if quiz_to_load.creator_id == current_user.get_db_id():
        return render_template("quiz_suggestions.html", suggestions=quiz_to_load.suggestions)
    else:
        return "Not Authorized", 401
